...
from gp_sinkhorn.channel_conv_kernel import ConvSimple
...
class RandomFourierFeatures:
    ...
    def __init__(self, x, y, num_features, kernel=RBF, noise=1, device=None,
        ...
        self.feature_mapping_fn = (
            ...
            self.feature_mapping_conv if self.conv else
    ...
    def init_params(self):
        ...
        if self.arccos:
            ...
            for patch_size in self.patch_sizes:
                num_patches = (29 - patch_size) ** 2
                len_input = num_patches * patch_size ** 2
                self.conv_ws.append(torch.normal(0, std_w, size=([len_input, self.num_features])).double().to(self.device))
                self.conv_bs.append(torch.normal(0, std_b, size=([self.num_features])).to(self.device))
        else:
    ...
    def feature_mapping_conv(self, x):
        ...
        phi = torch.zeros(x.shape[0], self.num_features).to(self.device)
        for i, patch_size in enumerate(self.patch_sizes):
            w = self.conv_ws[i]
            b = self.conv_bs[i]
            x_point_patches = self.flatten(extract_image_patches(x[:,:-1].reshape(-1, 1, 28, 28), patch_size))
            x1 = x_point_patches.flatten(start_dim=1).mm(w) + b
            phi += x1



        # x_point_patches = self.flatten(extract_image_patches(x[:,:-1].reshape(-1, 1, 28, 28), self.patch_size))
        # x1 = x_point_patches.flatten(start_dim=1).mm(self.w) + self.b

        # Scaling1 is the correct one; so we don't normalise by num_features
        scaling1 = torch.sqrt(self.variance) * math.sqrt(2) / self.num_features
        # scaling2 = torch.sqrt(self.variance) * math.sqrt(1 / self.num_features)
        # scaling3 = torch.sqrt(self.variance) * math.sqrt(2 / self.num_features)
        # scaling4 = torch.sqrt(self.variance) * math.sqrt(4 / self.num_features)

        return scaling1 * x1 # why x1 and not phi?
